...
**2025-11-08**\
About 40 minutes of studying. 8:42 pm. I fall asleep.\
The modification of the program turned out to be more difficult than I expected, so my brain tries to turn off instead of studying.

**2025-11-14**\
59 minutes of linear algebra (determinants), the information is from the video of 3Brown1Blue.

**2025-11-17**\
1 hour 36 minutes of studying.\
I loaded the image into my CNN generator and played a bit with dimensions of the FC and transposed convolution layers. The program works, but doesn't produce anything meaninful.\
Now I need to study the backpropagation process written by DeepSeek (ChatGPT or VPN don't work well enough). And the backprop for this program is rather new and big, though there are resemblances to the usual CNNs. DeepSeek is a bitch, but it can be very helpful, especially in programming.

**2025-11-22**\
1 hour 25 minutes.
The CNN generatoor program is rather difficult and intimidating, so I decided to watch `Essence of linear algebra` videos on 3Blue1Brown, the video `Inverse matrices, column space and null space | Chapter 7, Essence of linear algebra`.
Had an interesting conversation with DeepSeek about why it's impossible to recreate back a 2D matrix from 2D=>1D transformation. I understood it geometrically in the physical sense, but the mathematical understanding can be better.

**2025-11-24**\
About 40 minutes after work of studying the linear algebra (3Blue1Brown, transformations from 3D to 2D and opposite, relatively easy topic).\

**2025-11-30**\
Asked ChatGPT to write a CNN generator, then I tested if it works, fixed some minor problems. I want to make it really work before studying it, because it's a lot of efforts to study it, and I don't want to spend time digging through stuff that doesn't work.

**2025-12-29**\
My transposed CNN generator works. So the idea wasn't meaningless. Though actually it came up to me instinctively. An LLM suggested to make a CNN generator that does convolution and deconvolution. I asked:
- Why do I need convolution, if ideally deconvolution mirrors the output of convolution?\
The LLM aswered:
- Yep, it's true, you can omit convolution.\
So I came up with the idea: FFNN + deconvolution.\
Though I don't understand a bit of technical details (for example, the complicated loops that work with input channels, output channels, width and height simultaneously, but it doesn't seem to be profoundly difficult and impossible).

**2026-01-03**\
*The comments that I wrote under my last program. I need to write them in my diary, because they are important (though maybe difficult to undersand).*
...
I've created a pretty beutiful system to explain and implement backpropagation without thinking about dimensions. Here. This stuff have been fermenting in my brain I guess since I started to study ML. [my backprop discovery!](./003.%20my%20backprop%20discovery!.md) I got this idea while digging through my old ML pictures, saved from the internet.

**2026-01-11**\
About using different loops' order during application of convolutional kernels (or deconvolutional layers). Deepseek and Google AI say that difference in speed should be 10-1000x, but I don't see any difference in the output. Maybe my CPU cache is so huge that it handle all that flawlessly. So nevermind then, I'll just continue to study the CNN generoator:\
...
**2026-01-15**\
An interesting insight into this code (how to express it in words).
```Python
...
```
For each output (`out[cout]`) we take each input (`x[cin]`) and multiply it by each weight (`w[cin,cout]`). The good insight is that there's a connection between indices. `out[cout]` is a "single" variable (it has 1 index). `x[cin]` is a "single" variable too (with a single index (`[cin]`)). But `w[cin,cout]` has 2 indices: `cin` and `cout`, and they connect the weight with output and input. These 2 indices make the weight matrix (filter, kernel) reflect both the structure of ouput and the structure of input (because such kernel structure considers the number of output channels (`cout`) and the number of inputs (`cin`)).\
Basically the structure is:\
```
output channel += input channel * weight for this input channel and for this output channel
```
See how the weight structure naturally aligns with the demensions of input and output to supply them with all needed information (not more and not less).\
Also see, how each output channel sums up all the `x*w` multiplications.
~~If there are more output channels, then it just "hallucinates" outputs. If there are more input channels, the level of "hallucinations" is smaller (because there are more data in the input)~~. Actually DeepSeek told me that it's interesting, but not correct intuition. The more correct one is:
```Python
terminology = {
    'C_out > C_in': 'Feature expansion / Diversification',
    'C_in > C_out': 'Feature compression / Summarization',
    'balanced': 'Feature transformation / Representation learning'
}
```
I think the misuderstanding is mostly ethical and philosophical. I consider all neural network outputs "hallucinations", some are just beneficial/correct, and some are harmful/incorrect.\
But it says that summarization and feature expansion all have the potential for hallucination. And that summarization can be even more hallucinating (because it's not limited in the ability to distort/change information, while the feature expansion should save some of the initial data; I'm not sure if it's true, and generally such details are not very improtant for me now😁).\
Hmm this argument looks interesting:
```txt
Journalist (summarizer):
Takes: 8 hours of interviews, research
Produces: 800-word article
Must: INVENT narrative structure, emphasis, flow
Hallucinates: "What's the story here?"

Novelist (expander):
Takes: "Love story in Paris"
Produces: 300-page novel  
Adds: Characters, dialogue, scenes
But: Constrained by "love story in Paris"
```
